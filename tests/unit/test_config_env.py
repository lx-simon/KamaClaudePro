...
def test_llm_provider_from_toml(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "kama.toml"
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("KAMA_CONFIG", str(toml_path))
    monkeypatch.delenv("KAMA_LLM_PROVIDER", raising=False)
...
def test_llm_provider_env_overrides(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("KAMA_LLM_PROVIDER", "openai")
    monkeypatch.setenv("KAMA_LLM_DEFAULT_MODEL", "gpt-4o-mini")
