...
async def _openai_create_mock(**kwargs: object) -> FakeOpenAIStream:
    _openai_create_mock.kwargs = kwargs  # type: ignore[attr-defined]
    usage = MagicMock(prompt_tokens=12, completion_tokens=3)
    text_delta = MagicMock(content="Hello", tool_calls=None)
    ...
    tool_chunk = MagicMock(choices=[tool_choice], usage=None)
    usage_chunk = MagicMock(choices=[], usage=usage)
    return FakeOpenAIStream([text_chunk, tool_chunk, usage_chunk])
...
async def _openai_bad_index_create_mock(**kwargs: object) -> FakeOpenAIStream:
    ...
    tool_chunk = MagicMock(choices=[tool_choice], usage=None)
    usage_chunk = MagicMock(choices=[], usage=usage)
    return FakeOpenAIStream([tool_chunk, usage_chunk])
...
async def test_openai_provider_streams_text_and_tools() -> None:
    client = MagicMock()
    client.chat.completions.create = _openai_create_mock
    provider = OpenAIProvider(model="gpt-4o-mini", client=client)

    result, events = await _chat(
    ...
    tokens = [e for e in events if e.type == "llm.token"]  # type: ignore[attr-defined]
    assert tokens[0].token == "Hello"  # type: ignore[attr-defined]
    kwargs = _openai_create_mock.kwargs  # type: ignore[attr-defined]
    assert kwargs["tools"][0]["type"] == "function"  # type: ignore[index]
    assert kwargs["messages"][0]["role"] == "system"  # type: ignore[index]


async def test_openai_provider_ignores_invalid_tool_call_index() -> None:
    client = MagicMock()
    client.chat.completions.create = _openai_bad_index_create_mock
    provider = OpenAIProvider(model="gpt-4o-mini", client=client)

    result, _ = await _chat(
        provider,  # type: ignore[arg-type]
        messages=[{"role": "user", "content": "read it"}],
...
def test_missing_openai_api_key_raises_system_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    with pytest.raises(SystemExit):
        OpenAIProvider(model="gpt-4o-mini")


def test_create_provider_selects_openai(monkeypatch: pytest.MonkeyPatch) -> None:
