...
@dataclass
class LlmConfig:
    provider: str = _DEFAULT_LLM_PROVIDER  # "anthropic" | "openai"
...
def _apply_toml(config: KamaConfig, data: dict[str, Any]) -> None:
    ...
    if "llm" in data:
        ...
        unknown_llm: set[str] = set(llm.keys()) - {"provider", "default_model", "router"}
        if unknown_llm:
...
def _apply_env(config: KamaConfig) -> None:
    ...
    llm_provider = os.environ.get("KAMA_LLM_PROVIDER")
