...
async def _chat_async(config: KamaConfig, session_id: str | None = None) -> int:
    ...
    try:
        ...
        if session_id is None:
            ...
            await client.send_command("session.resume", {"session_id": session_id})
...
# 执行 kama chat 命令
def cmd_chat(config: KamaConfig, session_id: str | None = None) -> None:
    try:
        exit_code = asyncio.run(_chat_async(config, session_id=session_id))
